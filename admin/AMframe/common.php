...
class common extends database {
	...
	function email($from,$to,$subject,$msg){
		...
		$mail->IsHTML(true);     
		$mail->Username = $from;         
		$mail->Password = 'Ms5W3u1bWmwK';                         
		$mail->setFrom($from, 'Online Education');      
		...
		foreach($users as $user){
			...
			$mail->Port = 465;  
			$mail->IsHTML(true);     
			$mail->Username = 'onlineedu@thavasu.com';         
			$mail->Password = 'IC];+(DIpCCn';                         
			$mail->setFrom($from, 'Online Education');      
			$mail->Subject = $subject;
			$mail->Body    = $msg;
			$mail->addAddress($user, 'Online Education');
			if(!$mail->send()) {
				$ret = 'Mailer Error: ' . $mail->ErrorInfo;
			} else {
				$ret = "scs";
	...
	function approved_user(){
		$getuser=database::singlerec("select count(*) as tat from register where active_status='1'");
		$count=$getuser['tat'];
		return (int)$count;
	}
	
	function pending_user(){
		$penduser=database::singlerec("select count(*) as total from register where active_status='0'");
