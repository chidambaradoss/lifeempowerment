...
if($submit) {
	...
	$checkStatus = $db->check1column("partners","name",$name);
	if($upd==2)
		$checkStatus=0;
	...
	if($checkStatus == 0){
		...
		if($image!=''){
			$file_to=uniqid();
			$file=$com_obj->upload_image("image",$file_to,100,80,"../uploads/partner","NULL");
			if($file){
		...
		if($error=='') {
			if($upd == 1) {
				$set  .= ",crcdt = '$crcdt'";    
				$set  .= ",active_status = '1'";
				$db->insertrec("insert into partners set $set");
				$act = "add";
			}
			else if($upd == 2){
				$set  .= ",chngdt = '$crcdt'";   
				$db->insertrec("update partners set $set where id='$idvalue'");
				$act = "upd";
			}
			echo "<script>window.location='partners.php?act=$act'</script>"; 
			header("location:partners.php?act=$act");
			exit;
		}	
		else {
...
$GetRecord = $db->singlerec("select * from partners where id='$id'");
@extract($GetRecord);
				 ...
				 <div class="col-sm-6 eq-box-sm">
					<div class="panel">
						<div class="panel-heading">
							<h3 class="panel-title"><? echo $TextChange;?> Partner</h3>
						</div>
						<form class="form-horizontal" method="post" action="partnerupd.php" enctype="multipart/form-data">
							<input type="hidden" name="idvalue" value="<?echo $id;?>" />
							<input type="hidden" name="upd" value="<?echo $upd;?>" />							
							<div class="panel-body">
								<table style="padding:25px;">
									<tr>
										<td style="width:30%">Partner Name<font color="red">*</font></td>
							...
							<div class="panel-footer text-left">
								...
								<a class="btn btn-info" href="partners.php">Cancel</a>
							</div>
						</form>
						<!--===================================================-->
						<!--End Horizontal Form-->
